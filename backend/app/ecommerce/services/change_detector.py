...
class ChangeDetector:
    ...
    async def process_price_change(
        ...
        await self.product_service.add_price_history(
            product_id, new_price, currency, availability
        )
    ...
    async def _log_changes(
        ...
        if changes['price_changed']:
            ...
            if changes['significant_drop']:
                logger.info(f"Significant price drop detected: {changes['price_change_percent']:.1f}%")
    ...
    async def detect_availability_changes(
        self, 
        product_id: int, 
        ...
        was_out_of_stock = any(keyword in previous_availability for keyword in out_of_stock_keywords)
        is_out_of_stock = any(keyword in current_availability_lower for keyword in out_of_stock_keywords)
